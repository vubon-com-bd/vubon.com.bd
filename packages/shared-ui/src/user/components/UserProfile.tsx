/**
 * UserProfile Component
 * ইউজার প্রোফাইল কম্পোনেন্ট
 */

import React from 'react';
import { UserProfile as UserProfileType } from '@vubon/shared-types';
import { Card } from '../../common/components/Card';
import { UserAvatar } from './UserAvatar';

export interface UserProfileProps {
  profile: UserProfileType;
  onEdit?: () => void;
  className?: string;
}

export const UserProfile: React.FC<UserProfileProps> = ({
  profile,
  onEdit,
  className = '',
}) => {
  return (
    <Card className={`p-6 ${className}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-6">
          <UserAvatar
            name={profile.displayName || profile.fullName || 'User'}
            avatar={profile.avatar}
            size="xl"
          />
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {profile.displayName || profile.fullName}
            </h2>
            {profile.fullName && profile.fullName !== profile.displayName && (
              <p className="text-gray-500">{profile.fullName}</p>
            )}
            {profile.location && (
              <p className="text-gray-500">{profile.location}</p>
            )}
            {profile.company && profile.position && (
              <p className="text-gray-500">
                {profile.position} at {profile.company}
              </p>
            )}
          </div>
        </div>
        {onEdit && (
          <button
            onClick={onEdit}
            className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700"
          >
            Edit Profile
          </button>
        )}
      </div>

      {profile.bio && (
        <div className="mt-4 border-t border-gray-200 pt-4">
          <h3 className="text-sm font-medium text-gray-700">About</h3>
          <p className="mt-1 text-sm text-gray-600">{profile.bio}</p>
        </div>
      )}

      {profile.website && (
        <div className="mt-2">
          <h3 className="text-sm font-medium text-gray-700">Website</h3>
          <a
            href={profile.website}
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm text-blue-600 hover:underline"
          >
            {profile.website}
          </a>
        </div>
      )}

      <div className="mt-4 grid grid-cols-2 gap-4 border-t border-gray-200 pt-4">
        {profile.education && (
          <div>
            <h3 className="text-sm font-medium text-gray-700">Education</h3>
            <p className="mt-1 text-sm text-gray-600">
              {profile.education.charAt(0).toUpperCase() + profile.education.slice(1).replace('_', ' ')}
            </p>
          </div>
        )}
        {profile.employment && (
          <div>
            <h3 className="text-sm font-medium text-gray-700">Employment</h3>
            <p className="mt-1 text-sm text-gray-600">
              {profile.employment.charAt(0).toUpperCase() + profile.employment.slice(1).replace('_', ' ')}
            </p>
          </div>
        )}
        {profile.gender && (
          <div>
            <h3 className="text-sm font-medium text-gray-700">Gender</h3>
            <p className="mt-1 text-sm text-gray-600">
              {profile.gender.charAt(0).toUpperCase() + profile.gender.slice(1).replace('_', ' ')}
            </p>
          </div>
        )}
        {profile.dateOfBirth && (
          <div>
            <h3 className="text-sm font-medium text-gray-700">Date of Birth</h3>
            <p className="mt-1 text-sm text-gray-600">
              {new Date(profile.dateOfBirth).toLocaleDateString()}
            </p>
          </div>
        )}
      </div>
    </Card>
  );
};
