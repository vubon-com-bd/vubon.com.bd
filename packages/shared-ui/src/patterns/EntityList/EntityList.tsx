...
export const EntityList = forwardRef<HTMLUListElement, EntityListProps>(function EntityList(
  { items, emptyState, dividers = true, className, ...rest },
  ref
) {
  if (items.length === 0 && emptyState) return <>{emptyState}</>;
  return (
    <ul ref={ref} role="list" className={cn('flex flex-col', className)} {...rest}>
      {items.map((item) => {
        const Wrapper = item.href ? 'a' : item.onClick ? 'button' : 'div';
        return (
          <li key={item.id} className={cn(dividers && 'border-b border-slate-100 last:border-b-0')}>
            <Wrapper
