...
const authPermissionKeys = Object.keys(AUTH_PERMISSIONS) as [string, ...string[]];

export const AuthPermissionSchema = PermissionSchema.extend({
  permission: z.enum(authPermissionKeys),
  category: z.literal('auth'),
});

export const AuthPermissionEnumSchema = z.enum(authPermissionKeys);
