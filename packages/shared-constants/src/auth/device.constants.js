...
export const DEVICE_UTILS = {
     ...
     */
    getDeviceTypeFromUserAgent: (userAgent) => {
     ...
     */
    getDeviceVendor: (userAgent) => {
        const ua = userAgent.toLowerCase();
        if (ua.includes('apple'))
            return DEVICE_VENDORS.APPLE;
        if (ua.includes('samsung'))
            return DEVICE_VENDORS.SAMSUNG;
        if (ua.includes('huawei'))
            return DEVICE_VENDORS.HUAWEI;
        if (ua.includes('xiaomi'))
            return DEVICE_VENDORS.XIAOMI;
        if (ua.includes('oppo'))
            return DEVICE_VENDORS.OPPO;
        if (ua.includes('vivo'))
            return DEVICE_VENDORS.VIVO;
        if (ua.includes('realme'))
            return DEVICE_VENDORS.REALME;
        if (ua.includes('oneplus'))
            return DEVICE_VENDORS.ONEPLUS;
        if (ua.includes('nokia'))
            return DEVICE_VENDORS.NOKIA;
        if (ua.includes('symphony'))
            return DEVICE_VENDORS.SYMPHONY;
        if (ua.includes('walton'))
            return DEVICE_VENDORS.WALTON;
        if (ua.includes('transsion') || ua.includes('tecno') || ua.includes('infinix')) {
            return DEVICE_VENDORS.TRANSSION;
        }
        if (ua.includes('lenovo'))
            return DEVICE_VENDORS.LENOVO;
        if (ua.includes('dell'))
            return DEVICE_VENDORS.DELL;
        if (ua.includes('hp') || ua.includes('hewlett'))
            return DEVICE_VENDORS.HP;
        if (ua.includes('asus'))
            return DEVICE_VENDORS.ASUS;
        if (ua.includes('acer'))
            return DEVICE_VENDORS.ACER;
        if (ua.includes('microsoft') || ua.includes('surface'))
            return DEVICE_VENDORS.MICROSOFT;
        if (ua.includes('google') || ua.includes('pixel'))
            return DEVICE_VENDORS.GOOGLE;
        if (ua.includes('sony'))
            return DEVICE_VENDORS.SONY;
        if (ua.includes('lg'))
            return DEVICE_VENDORS.LG;
        if (ua.includes('panasonic'))
            return DEVICE_VENDORS.PANASONIC;
    ...
    getDeviceOS: (userAgent) => {
        ...
        if (ua.includes('chrome os'))
            return DEVICE_OS.CHROME_OS;
        if (ua.includes('tizen'))
            return DEVICE_OS.TIZEN;
        if (ua.includes('webos'))
            return DEVICE_OS.WEBOS;
        if (ua.includes('kaios'))
            return DEVICE_OS.KAIOS;
        if (ua.includes('freertos'))
            return DEVICE_OS.FREERTOS;
        if (ua.includes('windows phone') || ua.includes('wp'))
            return DEVICE_OS.WINDOWS_PHONE;
        if (ua.includes('fire os'))
            return DEVICE_OS.FIRE_OS;
        if (ua.includes('harmony'))
            return DEVICE_OS.HARMONY_OS;
        if (ua.includes('symbian'))
            return DEVICE_OS.SYMBIAN;
        if (ua.includes('blackberry'))
            return DEVICE_OS.BLACKBERRY;
        return DEVICE_OS.UNKNOWN;
    },
    /**
     * Get device browser from user agent
     */
    getDeviceBrowser: (userAgent) => {
        const ua = userAgent.toLowerCase();
        if (ua.includes('chrome'))
            return DEVICE_BROWSERS.CHROME;
        if (ua.includes('firefox'))
            return DEVICE_BROWSERS.FIREFOX;
        if (ua.includes('safari') && !ua.includes('chrome'))
            return DEVICE_BROWSERS.SAFARI;
        if (ua.includes('edge'))
            return DEVICE_BROWSERS.EDGE;
        if (ua.includes('opera mini'))
            return DEVICE_BROWSERS.OPERA_MINI;
        if (ua.includes('opera'))
            return DEVICE_BROWSERS.OPERA;
        if (ua.includes('brave'))
            return DEVICE_BROWSERS.BRAVE;
        if (ua.includes('vivaldi'))
            return DEVICE_BROWSERS.VIVALDI;
        if (ua.includes('samsung') && ua.includes('internet'))
            return DEVICE_BROWSERS.SAMSUNG_INTERNET;
        if (ua.includes('ucbrowser') || ua.includes('uc browser'))
            return DEVICE_BROWSERS.UC_BROWSER;
        if (ua.includes('nokia') && ua.includes('browser'))
            return DEVICE_BROWSERS.NOKIA_BROWSER;
        if (ua.includes('kaios'))
            return DEVICE_BROWSERS.KAIOS_BROWSER;
        if (ua.includes('webview'))
            return DEVICE_BROWSERS.WEBVIEW;
        return DEVICE_BROWSERS.UNKNOWN;
    },
    /**
     * Get device trust level based on factors
     */
    getTrustLevel: (device) => {
    ...
    isDeviceTrusted: (device) => {
        ...
        return (trustLevel === DEVICE_TRUST_LEVELS.TRUSTED || trustLevel === DEVICE_TRUST_LEVELS.VERIFIED);
    },
    /**
     * Check if device is suspicious
     */
    isDeviceSuspicious: (device) => {
        const trustLevel = DEVICE_UTILS.getTrustLevel(device);
        return (trustLevel === DEVICE_TRUST_LEVELS.SUSPICIOUS || trustLevel === DEVICE_TRUST_LEVELS.UNTRUSTED);
    },
    /**
     * Check if device is blocked
     */
    isDeviceBlocked: (device) => {
        return (device.isBlocked === true ||
     ...
     */
    getTrustLevelDisplayName: (level) => {
        const map = {
            trusted: '✅ Trusted',
...
//# sourceMappingURL=device.constants.js.map