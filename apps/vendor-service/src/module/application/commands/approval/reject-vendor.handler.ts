...
export class RejectVendorHandler
  extends BaseCommandHandler<RejectVendorCommand, ApprovalResponseDto>
  implements ICommandHandler<RejectVendorCommand>
{
  readonly commandType = 'vendor.approval.reject';
  ...
  async execute(command: RejectVendorCommand): Promise<ApprovalResponseDto> {
    ...
    const entity = VendorApprovalEntity.create({
      ...
      reason: ApprovalReasonVO.create(command.reason),
