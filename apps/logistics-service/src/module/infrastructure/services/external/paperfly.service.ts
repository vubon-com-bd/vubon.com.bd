...
export class PaperflyService extends BaseCourierService {
  ...
  async createShipment(input: CourierCreateShipmentInput): Promise<CourierCreateShipmentResult> {
    try {
      const data = await httpPost<{ tracking_number: string }>(
        `${PAPERFLY_CONFIG.baseUrl}/orders`,
        { ...input },
        {
          headers: { Authorization: `Bearer ${PAPERFLY_CONFIG.apiKey}` },
          timeoutMs: PAPERFLY_CONFIG.timeoutMs,
        },
      );
  ...
  async track(trackingNumber: string): Promise<CourierTrackResult> {
    try {
      const data = await httpGet<{ status: string }>(
        `${PAPERFLY_CONFIG.baseUrl}/track/${trackingNumber}`,
        {
          headers: { Authorization: `Bearer ${PAPERFLY_CONFIG.apiKey}` },
          timeoutMs: PAPERFLY_CONFIG.timeoutMs,
        },
  ...
  async cancel(trackingNumber: string): Promise<CourierCancelResult> {
    try {
      await httpPost(`${PAPERFLY_CONFIG.baseUrl}/cancel/${trackingNumber}`, {}, {
        headers: { Authorization: `Bearer ${PAPERFLY_CONFIG.apiKey}` },
      });
      return { success: true };
    } catch (error) {
