...
interface FedexWebhookDto {
  readonly trackingNumber: string;
  readonly status: string;
}

@Controller('webhooks/fedex')
export class FedexWebhookController {
  ...
  async handle(
    @Body() body: FedexWebhookDto,
    @Headers('x-fedex-signature') signature?: string,
  ): Promise<{ received: true }> {
    if (!signature) throw new UnauthorizedException('Missing signature');
    const secret = FEDEX_CONFIG.apiSecret ?? FEDEX_CONFIG.apiKey;
