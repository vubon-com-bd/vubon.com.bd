...
export class SteadfastWebhookController {
  ...
  async handle(
    @Body() body: SteadfastWebhookDto,
    @Headers('x-steadfast-signature') signature?: string,
  ): Promise<{ received: true }> {
    if (!signature) throw new UnauthorizedException('Missing signature');
    const secret = STEADFAST_CONFIG.apiSecret ?? STEADFAST_CONFIG.apiKey;
    ...
    await this.commandBus.execute(
      new UpdateShipmentCommand(body.tracking_code, body.status),
    );
