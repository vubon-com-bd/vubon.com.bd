...
export class MockTokenGenerator implements ITokenGenerator {
  ...
  private otps: Map<string, { code: string; expiresAt: Date; used: boolean; attempts: number }> = new Map();
  ...
  async generateOTP(options: OTPGenerationOptions): Promise<OTPGenerationResult> {
    const code = options.alphanumeric
      ? Math.random().toString(36).substring(2, 2 + (options.length || 6)).toUpperCase()
      : Math.random().toString(10).substring(2, 2 + (options.length || 6));
